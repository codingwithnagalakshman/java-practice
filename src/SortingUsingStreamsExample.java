...
public class SortingUsingStreamsExample {
    public static void main(String[] args) {
        ...
        List<Car> sortByMake = cars.stream().sorted(Comparator.comparing(Car::getMake)).collect(Collectors.toList());

        List<Car> sortByPrice = cars.stream().sorted(Comparator.comparingDouble(Car::getPrice)).collect(Collectors.toList());

        List<Car> sortByYear = cars.stream().sorted(Comparator.comparingInt(Car::getYear)).collect(Collectors.toList());


        System.out.println(sortByMake);
    }
}
