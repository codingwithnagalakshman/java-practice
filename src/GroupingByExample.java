...
public class GroupingByExample {
    public static void main(String[] args) {
        ...
        Map<Integer, List<Car>> collect = cars.stream().collect(Collectors.groupingBy(Car::getYear));
        System.out.println(collect);

        Map<Integer, Long> collect1 = cars.stream().collect(Collectors.groupingBy(Car::getYear, Collectors.counting()));
        System.out.println(collect1);

        Map<Integer, List<String>> collect2 = cars.stream().collect(Collectors.groupingBy(Car::getYear, Collectors.mapping(Car::getMake, Collectors.toList())));
        System.out.println(collect2);

        Map<Integer, Double> collect3 = cars.stream().collect(Collectors.groupingBy(Car::getYear, Collectors.summingDouble(Car::getPrice)));
