...
//        List<String> stringList = cars.stream().map(car -> car.getMake()).collect(Collectors.toList());
        List<String> stringList = cars.stream().map(Car::getMake).collect(Collectors.toList());
        ...
        List<Car> collect1 = cars.stream().map(car -> new Car(car.getMake(),
                        car.getModel(), car.getYear(), car.getPrice() + 10000))
